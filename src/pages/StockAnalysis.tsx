...
export const StockAnalysis: React.FC = () => {
  ...
  const handleExecuteTrade = (tradeType: "BUY" | "SELL" = "BUY") => {
    ...
    if (executionMode === "PAPER_TRADING") {
      ...
      let targetP = data.timingSignal.target1;
      let stopL = data.timingSignal.stopLoss;
          ...
          <>
                    ...
                    <div>
                      <h3 className="font-bold text-sm text-slate-100 flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-purple-500/20 text-purple-300 font-bold border border-purple-500/30">
                          {userProfile.tradingStyle.replace("_", " ")} · {userProfile.riskAppetite} RISK
                        </span>
                      </h3>
                      <p className="text-xs text-slate-400">
                    ...
                    <div className="mt-4 space-y-3">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs font-mono">
                        <div className="p-3 rounded-xl bg-slate-950/80 border border-slate-800">
                          ...
                          <span className="font-bold text-indigo-300">{userContext.purpose} ({userContext.timeHorizon})</span>
                        </div>
                        <div className="p-3 rounded-xl bg-slate-950/80 border border-slate-800">
                          <span className="text-[10px] text-slate-400 block mb-0.5">Interest Reason:</span>
                          <span className="font-bold text-purple-300">{userContext.interestReason.replace("_", " ")}</span>
                        </div>
                        <div className="p-3 rounded-xl bg-slate-950/80 border border-slate-800">
            ...
            <div id="section-overview" className="grid grid-cols-1 md:grid-cols-4 gap-4 scroll-mt-24">
              ...
              <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 flex flex-col justify-between">
                ...
                <div className="text-[11px] text-slate-400 truncate">
                  Market Sentiment: <span className="font-semibold text-slate-200">{data?.marketSentiment || "NEUTRAL"}</span>
                </div>
              </div>
            </div>
              ...
              return (
                <div id="section-timing" className="p-6 rounded-2xl bg-gradient-to-br from-indigo-950/40 via-slate-900 to-emerald-950/40 border border-indigo-500/30 shadow-2xl scroll-mt-24">
                  ...
                  <div className="p-3 rounded-lg bg-slate-950/60 text-xs text-slate-300 border border-slate-800/80">
                    <span className="font-semibold text-indigo-300">Timing Detector Signal:</span> {data.timingSignal.optimalTimingReason}
                  </div>
                </div>
              );
            })()}
            ...
            {/* Angel One SmartAPI Live Workstation Chart + Trade Order Execution Card */}
            <div id="section-chart" className="flex flex-col gap-4 scroll-mt-24">
                    ...
                    <div className="space-y-3 font-mono">
                      ...
                      <div className="p-3.5 rounded-xl bg-slate-950/80 border border-emerald-500/30 text-xs flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2 text-slate-300">
                          ...
                          <span>Execution Guardrail synchronized with Master AI Trading Brain v1 Card above.</span>
                        </div>
                        <button
                          onClick={() => handleExecuteTrade(data?.suggestedAction?.includes("SELL") ? "SELL" : "BUY")}
            ...
            {/* STEP 1: SCRAPED LAST 5 DAYS GOOGLE NEWS FEED CARD (OFFICIAL PRESS ONLY) */}
            ...
            {/* DEEP HISTORICAL STOCK KUNDLI CARD (5Y, 10Y, LIFETIME INCEPTION ANALYSIS) */}
            {data?.stockKundli && (
              <div id="section-kundli" className="p-6 rounded-2xl bg-slate-900/90 border border-slate-800 scroll-mt-24">
                <div className="flex items-center justify-between pb-4 mb-4 border-b border-slate-800">
                  <div className="flex items-center gap-2.5">
                    <span className="p-2 rounded-xl bg-amber-500/20 text-amber-400 border border-amber-500/30">
                    ...
                    <div>
                      <h3 className="font-bold text-base text-slate-100">
                        Stock Lifetime Kundli (5Y, 10Y, Inception Analysis)
                      </h3>
                      <p className="text-xs text-slate-400">
                        {data.stockKundli.historicalRegime}
                      </p>
                    </div>
                  </div>
                  <span className="text-xs font-mono px-2.5 py-1 rounded bg-amber-500/10 text-amber-300 border border-amber-500/20">
                    Listed: {data.stockKundli.inceptionYear}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                  {/* ATH */}
                  <div className="p-3 rounded-xl bg-slate-950/80 border border-slate-800">
                    <span className="text-[11px] text-slate-400 block mb-1">All-Time High (ATH):</span>
                    <div className="text-base font-bold font-mono text-emerald-400">
                      ₹{data.stockKundli.allTimeHigh.toLocaleString()}
                    </div>
                    <span className="text-[10px] text-slate-500 font-mono">{data.stockKundli.allTimeHighDate}</span>
                  </div>

                  {/* ATL */}
                  <div className="p-3 rounded-xl bg-slate-950/80 border border-slate-800">
                    <span className="text-[11px] text-slate-400 block mb-1">All-Time Low (ATL):</span>
                    <div className="text-base font-bold font-mono text-rose-400">
                      ₹{data.stockKundli.allTimeLow.toLocaleString()}
                    </div>
                    <span className="text-[10px] text-slate-500 font-mono">{data.stockKundli.allTimeLowDate}</span>
                  </div>

                  {/* 5Y CAGR */}
                  <div className="p-3 rounded-xl bg-slate-950/80 border border-slate-800">
                    <span className="text-[11px] text-slate-400 block mb-1">5-Year CAGR Return:</span>
                    <div className="text-base font-bold font-mono text-indigo-300">
                      +{data.stockKundli.cagr5YearPct}% / yr
                    </div>
                  ...
                  {/* Max Drawdown */}
                  <div className="p-3 rounded-xl bg-slate-950/80 border border-slate-800">
                    <span className="text-[11px] text-slate-400 block mb-1">Max Lifetime Drawdown:</span>
                    <div className="text-base font-bold font-mono text-amber-300">
                      -{data.stockKundli.maxDrawdownPct}%
                    </div>
                  ...
                  <div className="p-3 rounded-xl bg-slate-950/80 border border-slate-800 col-span-2 sm:col-span-1">
                    <span className="text-[11px] text-slate-400 block mb-1">Historical History Depth:</span>
                    <div className="text-base font-bold font-mono text-purple-300">
                      {data.stockKundli.lifetimeBarCount} Months
                    </div>
                    <span className="text-[10px] text-slate-500">Full Monthly History</span>
                  </div>
                </div>
              </div>
            )}

            {/* DECISION AUDIT TRAIL & PERSONAL TRADING JOURNAL CARD */}
            <div id="section-journal" className="p-6 rounded-2xl bg-slate-900/90 border border-slate-800 scroll-mt-24">
              <div className="flex items-center justify-between pb-4 mb-4 border-b border-slate-800">
                <div className="flex items-center gap-2.5">
                  <span className="p-2 rounded-xl bg-indigo-500/20 text-indigo-400 border border-indigo-500/30">
                    <History className="w-5 h-5" />
                  ...
                  <div>
                    <h3 className="font-bold text-base text-slate-100">
                      Personal Rupee Decision Audit Trail & Performance Journal
                    </h3>
                    <p className="text-xs text-slate-400">
                      Review exact historical recommendation setups in Indian Rupees (₹), track win/loss outcomes, and evaluate discipline
                    </p>
                  </div>
                </div>
                <span className="text-xs font-mono text-indigo-300 bg-indigo-500/10 px-2.5 py-1 rounded border border-indigo-500/20">
                  {auditLogs.length} Decisions Logged
                </span>
              </div>
                ...
                <table className="w-full text-left text-xs">
                  <thead>
                    <tr className="border-b border-slate-800 text-slate-400 uppercase text-[10px] font-mono">
                      ...
                      <th className="py-2.5 px-3 text-right">Feedback / Journal</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-800/60 text-slate-300 font-mono">
                    {auditLogs.map((log) => (
                      <tr key={log.id} className="hover:bg-slate-950/40 transition">
                        <td className="py-2.5 px-3 text-[11px] text-slate-400">{log.timestamp}</td>
                        <td className="py-2.5 px-3 font-bold text-indigo-300">{log.ticker}</td>
                        <td className="py-2.5 px-3">₹{log.entryPrice.toLocaleString()}</td>
                        <td className="py-2.5 px-3">
                          <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${getActionBadge(log.action)}`}>
                            {log.action}
                          </span>
                        </td>
                        <td className="py-2.5 px-3">{log.confidenceScore}%</td>
                        <td className="py-2.5 px-3 text-[10px]">
                          <span className={`px-1.5 py-0.5 rounded ${log.mode === "PAPER_TRADING" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-amber-500/10 text-amber-300 border border-amber-500/20"}`}>
                            {log.mode === "PAPER_TRADING" ? "PAPER" : "LIVE"}
                          </span>
                        </td>
                        <td className="py-2.5 px-3">
                          <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${log.userOutcome === "WIN" ? "bg-emerald-500/20 text-emerald-400" : log.userOutcome === "LOSS" ? "bg-rose-500/20 text-rose-400" : "bg-slate-800 text-slate-400"}`}>
                            {log.userOutcome || "PENDING"}
                          </span>
                        </td>
                        <td className="py-2.5 px-3 text-right">
                          <div className="flex items-center justify-end gap-1.5">
                            <button
                              onClick={() => handleFeedback(log.id, "HELPFUL", "WIN")}
                              className={`p-1.5 rounded transition ${log.userFeedback === "HELPFUL" ? "bg-emerald-500/30 text-emerald-300" : "bg-slate-800 text-slate-400 hover:text-white"}`}
                              title="Mark Trade Win / Helpful"
                            >
                              <ThumbsUp className="w-3.5 h-3.5" />
                            </button>
                            <button
                              onClick={() => handleFeedback(log.id, "UNHELPFUL", "LOSS")}
                              className={`p-1.5 rounded transition ${log.userFeedback === "UNHELPFUL" ? "bg-rose-500/30 text-rose-300" : "bg-slate-800 text-slate-400 hover:text-white"}`}
                              title="Mark Trade Loss / Unhelpful"
                            >
            ...
            {/* 4 Expert Analyst Module Breakdown Cards */}
            <div id="section-modules" className="p-6 rounded-2xl bg-slate-900/90 border border-slate-800 scroll-mt-24">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-800">
                <div>
                  <h3 className="font-bold text-lg text-slate-200">Expert Analyst Module Signals</h3>
                  <p className="text-xs text-slate-400">Independent signals prior to multi-agent orchestration</p>
                </div>
                <div className="flex items-center gap-1.5 bg-slate-950 p-1 rounded-lg border border-slate-800 text-xs">
                  <button
                    onClick={() => setActiveTab("all")}
                    className={`px-3 py-1 rounded-md transition ${activeTab === "all" ? "bg-indigo-600 text-white font-medium" : "text-slate-400 hover:text-white"}`}
                  >
                    All Modules
                  </button>
                  <button
                    onClick={() => setActiveTab("technical")}
                    className={`px-3 py-1 rounded-md transition ${activeTab === "technical" ? "bg-indigo-600 text-white font-medium" : "text-slate-400 hover:text-white"}`}
                  >
                    Technical
                  </button>
                  <button
                    onClick={() => setActiveTab("fundamental")}
                    className={`px-3 py-1 rounded-md transition ${activeTab === "fundamental" ? "bg-indigo-600 text-white font-medium" : "text-slate-400 hover:text-white"}`}
                  >
                    Fundamental
                  </button>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Technical Module */}
                {(activeTab === "all" || activeTab === "technical") && data?.technicalAnalysis && (
                  <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800/80">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <BarChart2 className="w-4 h-4 text-indigo-400" />
                        <span className="font-bold text-sm text-slate-200">Technical Module</span>
                      </div>
                      <span className="text-xs font-mono font-semibold px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-400">
                        {data.technicalAnalysis.signal.toUpperCase()} ({data.technicalAnalysis.confidence}%)
                      </span>
                    </div>
                    <p className="text-xs text-slate-300 mb-3">{data.technicalAnalysis.summary}</p>
                    <div className="space-y-1 text-[11px] text-slate-400">
                      {data.technicalAnalysis.evidence?.map((ev, i) => (
                        <div key={i}>• {ev}</div>
                      ))}
                    </div>
                  </div>
                ...
                {/* Fundamental Module */}
                {(activeTab === "all" || activeTab === "fundamental") && data?.fundamentalAnalysis && (
                  <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800/80">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <PieChart className="w-4 h-4 text-purple-400" />
                        <span className="font-bold text-sm text-slate-200">Fundamental Module</span>
                      </div>
                      <span className={`text-xs font-mono font-semibold px-2 py-0.5 rounded ${data.fundamentalAnalysis.signal === "bullish" ? "bg-emerald-500/20 text-emerald-400" : "bg-rose-500/20 text-rose-400"}`}>
                        {data.fundamentalAnalysis.signal.toUpperCase()} ({data.fundamentalAnalysis.confidence}%)
                      </span>
                    </div>
                    <p className="text-xs text-slate-300 mb-3">{data.fundamentalAnalysis.summary}</p>
                    <div className="space-y-1 text-[11px] text-slate-400">
                      {data.fundamentalAnalysis.evidence?.map((ev, i) => (
                        <div key={i}>• {ev}</div>
                      ))}
                    </div>
                  </div>
                ...
                {/* Sentiment Module */}
                {activeTab === "all" && data?.newsAnalysis && (
                  <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800/80">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <FileText className="w-4 h-4 text-blue-400" />
                        <span className="font-bold text-sm text-slate-200">Sentiment Module (NLP)</span>
                      </div>
                      <span className="text-xs font-mono font-semibold px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-400">
                        {data.newsAnalysis.signal.toUpperCase()} ({data.newsAnalysis.confidence}%)
                      </span>
                    </div>
                    <p className="text-xs text-slate-300 mb-3">{data.newsAnalysis.summary}</p>
                    <div className="space-y-1 text-[11px] text-slate-400">
                      {data.newsAnalysis.evidence?.map((ev, i) => (
                        <div key={i}>• {ev}</div>
                      ))}
                    </div>
                  </div>
                ...
                {/* Macro & Sector Module */}
                {activeTab === "all" && data?.macroAnalysis && (
                  <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800/80">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <Globe className="w-4 h-4 text-amber-400" />
                        <span className="font-bold text-sm text-slate-200">Macro & Sector Module</span>
                      </div>
                      <span className="text-xs font-mono font-semibold px-2 py-0.5 rounded bg-amber-500/20 text-amber-400">
                        {data.macroAnalysis.signal.toUpperCase()} ({data.macroAnalysis.confidence}%)
                      </span>
                    </div>
                    <p className="text-xs text-slate-300 mb-3">{data.macroAnalysis.summary}</p>
                    <div className="space-y-1 text-[11px] text-slate-400">
                      {data.macroAnalysis.evidence?.map((ev, i) => (
                        <div key={i}>• {ev}</div>
                      ))}
                    </div>
                  </div>
            ...
            {/* Synthesized LLM Reasoning & Data Gaps */}
            <div className="p-6 rounded-2xl bg-slate-900/90 border border-slate-800 space-y-4">
              <div>
                <h3 className="font-bold text-base text-slate-200 mb-1">Synthesized Reasoning Thesis</h3>
              ...
              <div className="pt-3 border-t border-slate-800/80 flex flex-col sm:flex-row items-start sm:items-center justify-between text-xs text-slate-400 gap-2">
                <div>
                  <span className="font-semibold text-slate-300">Historical Analogue:</span> {data?.historicalSimilarity}
                </div>
                <div>
                  <span className="font-semibold text-rose-400">Data Missing:</span> {data?.dataMissing?.join(" · ")}
                </div>
              </div>
            </div>
